...
class TestLoggingFunctions(unittest.TestCase):
    ...
    def test_send_daily_logs_success(
        self, mock_clear_logs, mock_open, mock_exists, mock_logger, mock_send_email
    ):
        ...
        self.assertIn(
            "Successfully sent email with log:",
    ...
    @patch("utils.logs_utils.os.path.exists")
    @patch("utils.logs_utils.open", new_callable=mock_open)
    def test_send_daily_logs_error_handling(
        self, mock_open, mock_exists, mock_logger, mock_send_email
    ):
        mock_exists.return_value = True
        mock_open.return_value.read.return_value = "Log content here"
        ...
        self.assertIn(
            ...
            [call[0][1] for call in mock_logger.warning.call_args_list],
        )
    ...
    @patch("utils.logs_utils.os.path.exists")
    @patch("utils.logs_utils.open", new_callable=mock_open)
    def test_clear_logs_error_handling(
        self, mock_open, mock_exists, mock_logger, mock_send_email
    ):
        mock_exists.return_value = True
        mock_send_email.side_effect = Exception("Failed to send email")
