...
def handle_ta_df_initial_praparation(
    df: pd.DataFrame, settings: TechnicalAnalysisSettings
...
def calculate_ta_rsi(df: pd.DataFrame, settings: object) -> Union[pd.DataFrame, bool]:
    ...
    Returns:
        ...
        bool: False if an error occurs.
    """
    df["rsi"] = talib.RSI(df["close"], timeperiod=settings.rsi_timeperiod)
...
def calculate_ta_cci(
    ...
    """
    df["cci"] = talib.CCI(
        df["high"], df["low"], df["close"], timeperiod=settings.cci_timeperiod
    )
...
def calculate_ta_adx(
    ...
    df["adx"] = talib.ADX(
        df["high"], df["low"], df["close"], timeperiod=settings.adx_timeperiod
    )
...
def calculate_ta_atr(
    ...
    """
    df["atr"] = talib.ATR(
        df["high"], df["low"], df["close"], timeperiod=settings.atr_timeperiod
    )
...
def calculate_ta_di(
    ...
    df["plus_di"] = talib.PLUS_DI(
        df["high"], df["low"], df["close"], timeperiod=settings.di_timeperiod
    )
...
def calculate_ta_ema(
    ...
    Returns:
        ...
        bool: False if an error occurs during calculation.
    """
    df["ema_fast"] = talib.EMA(df["close"], timeperiod=settings.ema_fast_timeperiod)

    df["ema_slow"] = talib.EMA(df["close"], timeperiod=settings.ema_slow_timeperiod)
...
def calculate_ta_stochastic_rsi(
    ...
    Returns:
        ...
        bool: False if an error occurs during calculation.
    """
    df["stoch_rsi"] = talib.RSI(df["rsi"], timeperiod=settings.stoch_rsi_timeperiod)
...
def handle_ta_df_final_cleaning(
    df: pd.DataFrame, columns_to_check: list, settings: TechnicalAnalysisSettings
) -> Union[pd.DataFrame, bool]:
    """
...
def check_ta_trend(df: pd.DataFrame, settings: TechnicalAnalysisSettings) -> str:
    ...
    adx_trend = float(latest_data["adx"]) > float(settings.adx_strong_trend) or float(
        latest_data["adx"]
    ) > float(avg_adx)
    ...
    di_difference_increasing = abs(
        float(latest_data["plus_di"]) - float(latest_data["minus_di"])
    ) > abs(float(avg_plus_di) - float(avg_minus_di))

    significant_move = (float(latest_data["high"]) - float(latest_data["low"])) > float(
        latest_data["atr"]
    )
    ...
    uptrend = (
        is_rsi_bullish
        and adx_trend
        and di_difference_increasing
        and is_strong_plus_di
        and significant_move
        and float(latest_data["plus_di"]) > float(avg_minus_di)
    )
    ...
    downtrend = (
        ...
        and adx_trend
        and di_difference_increasing
        and is_strong_minus_di
        and significant_move
        and float(latest_data["plus_di"]) < float(avg_minus_di)
    )

    horizontal = (
        float(latest_data["adx"]) < avg_adx
        or avg_adx < float(settings.adx_weak_trend)
        or abs(float(latest_data["plus_di"]) - float(latest_data["minus_di"]))
