...
class HunterViewsTest(TestCase):
    ...
    def test_hunter_create_or_edit_create(self):
        ...
        data = {"name": "Hunter 1", "df": '{"price": [1, 2, 3]}'}
    ...
    def test_hunter_create_or_edit_edit(self):
        hunter = TechnicalAnalysisHunter.objects.create(
            user=self.user, name="Hunter 1", df='{"price": [1, 2, 3]}'
        )
        url = reverse("hunter:hunter_create_or_edit", kwargs={"pk": hunter.pk})
        data = {"name": "Updated Hunter", "df": '{"price": [4, 5, 6]}'}
        response = self.client.post(url, data)
        self.assertRedirects(response, reverse("hunter:show_hunters_list"))
    ...
    def test_hunter_delete(self):
        hunter = TechnicalAnalysisHunter.objects.create(
            user=self.user, name="Hunter to delete", df='{"price": [1, 2, 3]}'
        )
    ...
    def test_remove_all_hunters(self):
        TechnicalAnalysisHunter.objects.create(
            user=self.user, name="Hunter 1", df='{"price": [1, 2, 3]}'
        )
        TechnicalAnalysisHunter.objects.create(
            user=self.user, name="Hunter 2", df='{"price": [4, 5, 6]}'
        )
    ...
    def test_start_all_hunters(self):
        hunter1 = TechnicalAnalysisHunter.objects.create(
            user=self.user, name="Hunter 1", df='{"price": [1, 2, 3]}'
        )
        hunter2 = TechnicalAnalysisHunter.objects.create(
            user=self.user, name="Hunter 2", df='{"price": [4, 5, 6]}'
        )
    ...
    def test_stop_hunter(self):
        hunter = TechnicalAnalysisHunter.objects.create(
            user=self.user, name="Hunter 1", df='{"price": [1, 2, 3]}', running=True
        )
    ...
    def test_stop_all_hunters(self):
        ...
        hunter2 = TechnicalAnalysisHunter.objects.create(
            user=self.user, name="Hunter 2", df='{"price": [4, 5, 6]}', running=True
        )
